...
public:
    GraspGenerator() : nh_("~"), it_(nh_)
    {
        ...
        // 订阅
        cam_info_sub_ = nh_.subscribe("/camera/depth/camera_info",   1, &GraspGenerator::camInfoCallback, this);
        depth_sub_    = it_.subscribe("/camera/depth/image_raw",     1, &GraspGenerator::depthCallback, this);
        det_sub_      = nh_.subscribe("/object_detections",         10, &GraspGenerator::detectionCallback, this);

        // 发布
        grasp_pub_ = nh_.advertise<grasp_generator::GraspPose>("/grasp_poses", 10);

        ROS_INFO("Grasp Generator Node Initialized");
    }
    ...
    void detectionCallback(const grasp_generator::ObjectDetection2DConstPtr& msg)
    {
        ...
        try
        {
            ...
            cv_bridge::CvImagePtr mask_ptr = cv_bridge::toCvCopy(msg->mask, "mono8");

            // 生成抓取位姿
            Matrix4f grasp_pose;
            float width, score;
            if (generateGrasp(bbox, mask_ptr->image, cv_depth_ptr_->image, grasp_pose, width, score))
            {
                // 创建抓取消息
    ...
    bool generateGrasp(const std::vector<int>& bbox, const cv::Mat& mask, const cv::Mat& depth_map,
                      Matrix4f& grasp_pose, float& grip_width, float& stability_score)
    {
        ...
        // 提取掩码区域内的深度值
        cv::Mat masked_roi = cv::Mat::zeros(roi_height, roi_width, CV_32FC1);
        ...
        for (int y = 0; y < roi_height; y++)
        {
            for (int x = 0; x < roi_width; x++)
            {
                ...
                {
                    float d = depth_map.at<float>(y1 + y, x1 + x);
                    if (d > 0)
                    {
                        masked_roi.at<float>(y, x) = d;
        ...
        {
            ROS_WARN("No valid depth points in mask");
            return false;
        }
        ...
        Vector3f center_3d = pixelToCamera(center_u, center_v, median_depth);
        ...
        for (int y = 0; y < roi_height; y++)
        {
            for (int x = 0; x < roi_width; x++)
            {
                if (masked_roi.at<float>(y, x) > 0)
                {
                    Vector3f p = pixelToCamera(x1 + x, y1 + y, masked_roi.at<float>(y, x));
                    points.push_back(p);
                }
            }
        }
        ...
        if (points.size() < 3)
        {
            ROS_WARN("Not enough points for PCA");
            return false;
        }
        ...
        // 构建平移矩阵 (稍微抬升)
        Vector3f translation = center_3d + grasp_offset_ * z_axis;
        ...
        depth_variance = (sum_sq - sum * sum / valid_depths.size()) / valid_depths.size();

        float coverage = valid_depths.size() / static_cast<float>(roi_width * roi_height);
